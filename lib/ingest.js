...
class DataIngester {
  ...
  async ingest () {
    ...
    try {
      ...
      // Process downloads with limited concurrency to avoid blocking event loop
      const CONCURRENCY = 10
      for (let i = 0; i < toDownload.length; i += CONCURRENCY) {
        const batch = toDownload.slice(i, i + CONCURRENCY)
        await Promise.all(batch.map(async ({ date, url }) => {
          try {
        ...
        this.logger.info({ batch: Math.floor(i / CONCURRENCY) + 1, totalBatches: Math.ceil(toDownload.length / CONCURRENCY) }, 'Processed batch')
      }
